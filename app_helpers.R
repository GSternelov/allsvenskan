  ...
  if(conut_aes == 0){
    p = smPlot + 
      geom_point(data = data[situation2 != "penalty" & player.shortName == playerName],
                 alpha = .75, size = 4, shape = 21, stroke = 2, fill = "indianred3")
  }else if(conut_aes == 1){
    if(sColor != ""){
      p = smPlot + 
        geom_point(data = data[situation2 != "penalty" & player.shortName == playerName],
                   alpha = .75, size = 4, aes(x = 105 - x2, y = 68 - y2, color = .data[[sColor]]), inherit.aes = FALSE) +
        scale_color_viridis(discrete = TRUE, end = .9)
    }else if(sShape != ""){
      p = smPlot + 
        geom_point(data = data[situation2 != "penalty" & player.shortName == playerName],
                   alpha = .75, size = 4, fill = "indianred3", stroke = 2,
        ...
        p = smPlot + 
          ...
          scale_size(range = c(0.5, 8), limits = c(0,1))
    }
  }else if(conut_aes == 2){
    if(sColor == ""){
      p = smPlot +
